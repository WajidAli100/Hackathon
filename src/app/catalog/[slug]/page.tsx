...
import { oneProductType, responseType } from "@/components/utils/ProductsDataArraysAndTypes";
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
    ...
    const product = await fetch(`https://70iorhhu.api.sanity.io/v2023-07-28/data/query/production?query=*[_type == 'products']`).then((res: any) => res.json());
...
async function fetchPreviewData(slug: string) {
    let res = await fetch(`https://70iorhhu.api.sanity.io/v2023-07-28/data/query/production?query=*%5B_type%20%3D%3D%20%22products%22%20%26%26%20slug.current%3D%3D%20%22${slug}%22%5D`)
    return res.json();
};
...
export async function generateStaticParams() {
    let res = await fetch(`https://70iorhhu.api.sanity.io/v2023-07-28/data/query/production?query=*[_type == 'products']`, {
        next: {
            revalidate: 60
        }
